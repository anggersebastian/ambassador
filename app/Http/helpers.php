...
if(!function_exists('defaultResponseApi')){
    function defaultResponseApi($data = []){
        $user   = false;
        if(!empty($data['user']) AND !empty($data['user']->id)){
            $user   = $data['user'];
        }
        ...
        return [
            ...
            'token'     => ($user ? $user->remember_token : ''),
            'data'      => $data
        ];
    }
}

if(!function_exists('formatingDefaultDate')){
    function formatingDefaultDate($m, $type){
        if($type) return $m. ' 23:59:59';
        return $m. ' 00:00:00';
    };
}
