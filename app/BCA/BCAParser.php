...
class BCAParser
{
    ...
    function __construct( $conf )
    {
        ...
        $start      = mktime( $d[3], $d[4], $d[5], $d[1], ( $d[2] - 2 ), $d[0] );

        $this->explode_date	= $d;
        $this->post_time['end']['y'] = $d[0];
        $this->post_time['end']['m'] = ($d[1] < 10) ? (int)$d[1] : $d[1];
        $this->post_time['end']['d'] = $d[2];
        $this->post_time['start']['y'] = date( 'Y', $start );

        $dateStart	= date( 'm', $start );
        $this->post_time['start']['m'] = ($dateStart < 10) ? (int)$dateStart : $dateStart;
